...
<div align=justify style="text-indent: 3em;"> Durante a pandemia do Coronavírus e em plena era dos jogos "*digitais*", observa-se cada vez mais os chamados jogos *"analógicos"* (jogos de mesa e de tabuleiro) retornando às prateiras das lojas e *sites* especializados. Segundo dados da [*"Euromonitor International*](https://www.dicebreaker.com/categories/board-game/news/games-and-puzzles-market-2020-2021), empresa britânica de pesquisa de mercado,  a combinação de tempo livre e nostalgia fez com que os europeus voltassem a procurá-los como opção de lazer e entretenimento. No Brasil, este comportamento também se repete. O mercado de brinquedos e jogos tradicionais aumentou de 1% do período pré-pandêmico para 3,16% em 2022.
...
O trabalho final do curso de Visualizações I tem por objetivo analisar uma base de dados e construir visualizações gráficas a partir de seus dados. 

Uma das sugestões do curso foi a base de dados "Board Games" do *site* "Kaggle". A plataforma "Tidytuesday" utilizou este *dataset* em um dos seus eventos semanais. Resolvi, então, usá-la para a elaboração deste trabalho.

O TidyTuesday é um evento semanal R4DS Online for Data Science #TidyTuesday. que desafia os usuários da linguagem R a construir visualizações sobre o tema e assim promover discussões saudáveis. Os usuários podem, assim,  obter *feedback*, aprender com o trabalho de outras pessosas e se conectar com a comunidade do R. 

A base  *"Board Games"* é composta por dois arquivos de dados: *"ratings"* e *"details"*, que trazem informações sobre jogos de tabuleiros como **ranking**, nomes dos jogos, ano de lançamento, nota média e a nota média bayesiana, número de jogadores, entre outros dados.

As visualizações que apresentarei neste relatório foram criadas pensando nas pessoas que, em algum momento, precisam de um jogo de que permita ser jogado por um jogador ou mais, chamados de **"solo board games"**. Não são raros, principalmente neste período pandêmico, momentos em que nos encontramos sozinhos e gostaríamos de um passatempo para ocupar nossas mentes.
...
## Melhores do Ranking e médias

<div align=justify style="text-indent: 3em;">As vezes as pessoas ficam sozinhas em casa e gostariam de ter um jogo de tabuleiro para se divertir, ou simplesmente adoram jogar sozinhas. Para estas pessoas elaboramos os gráficos que seguirão. Ele é conhecido como *"lollipop plot"*, um tipo de um gráfico de barras, mas com as barras  substitúidas por uma linha e um ponto em sua extremidade.

O gráfico em questão apresenta os jogos que permitem ao usuário brincar sozinho ou com mais jogadores. Eles estão ordenados pela sua posição no ranking geral entre todos os jogos e trazem as respectivas notas médias bayesianas.

Nesta análise, consideramos apenas os jogos com mais de 10000 avaliações e selecionarmos, apenas 20 jogos.
...
## O mais desejados e ranking

<div align=justify style="text-indent: 3em;">Digamos que você não confie muito *rankings*, então vamos analisar quais são os mais desejados entre os jogadores e suas posições no *"ranking"* geral de jogos.

Vamos considerar novamente apenas os jogos com mais de 10000 avaliações e selecionaremos os vinte jogos mais desejados.
...
<div align=justify style="text-indent: 3em;"> Vimos que nem todos os jogos do topo do *ranking* são os mais desejados pelos usuparios que participaram das avaliações. Fatores como preço, tempo mínimo de jogo, categoria, mecânica do jogo, faixa etária afetam o desejo em adquirir um ou outro jogo. 
Por este motivo há a diferença entre as variáveis "desejar" e "querer". Nem sempre o jogo que colocamos em uma lista de desejos (wishlist) é também aquele que colocamos em uma lista de procurados quando estamos camprando o jogo. 

Considerando que: 1)  um tema que não chama a atenção do jogador o torna desinteressante, é preciso escolher bem entre os vários temas disponiveis como aventura, terror, médico, animais, ficção científica e até inspirados em outras obras como livros; 2) o tempo de jogo é importante para quem joga sozinho, então  um jogo que demore em média 4 horas talvez não seja o que você deseja; 3) quem vai jogar é uma criança, deve-se observar a idade mínima. Muitos jogos exigem um raciocínio analítico e uso de estratégias avançadas, por este motivo os fabricantes informam a idade mínima recomendada e 4) a "rejogabilidade", ou seja, jogar novamente com novas aventuras é importante, então é preciso que haja extensões de jogos.


Então, vamos considerar estas varíaveis nos gráficos que seguirão.
...
jogos_tabuleiro_categoria |> 
    ...
    theme(                                                       # altera as características do tema
        ...
        legend.box.background = element_rect(color = "#002642"),
    ...
    annotate("rect", xmin = 18700, xmax = 19600, ymin = 30, ymax = 30.6,
             fill = "#C1EEFF", alpha = 1, color = "#000000", size = 0.3)+
...
<div align=justify style="text-indent: 3em;"> Os tempos de jogos (mínimo e máximo) estão expressos em minutos. Trata-se de um tempo médio de jogo, haja vista que o tempo de duração depende de fatores como por exemplo o números de jogadores e não considera o tempo de *setup*, aquele gasto para retirar todas a peças e dispô-las no tabuleiro. 

O tempo de jogo é um fator importante na escolha, como também a categoria, a mecânica, a faixa etária, etc.



Entretanto, alguns jogos como "Robinson Crusoe - Adventures on the Cursed Island", "The 7th Continent" e "Le Havre" 
</div>
