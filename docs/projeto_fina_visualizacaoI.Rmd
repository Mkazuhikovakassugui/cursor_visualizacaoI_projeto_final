...
<div align=justify style="text-indent: 3em;"> Durante a pandemia da covid-19 e em plena era dos jogos "digitais", observa-se uma crescente procura pelos chamados jogos "analógicos", jogos de mesa e de tabuleiro, que retornaram às prateiras das lojas e *sites* especializados. Segundo dados da *Euromonitor International*, empresa britânica de pesquisa de mercado,  a combinação de tempo livre e nostalgia fez com que os europeus voltassem a procurá-los como opção de entretenimento. No Brasil, este comportamento também se repete. O mercado de brinquedos e jogos tradicionais aumentou de 1% do período pré-pandêmico para 3,16% em 2022.

Isto tem mexido com indústria de "games" eletrônicos e de mesa, tanto que jogos como [*"Assassin's  Creed"*](https://olhardigital.com.br/2022/03/17/games-e-consoles/jogo-de-tabuleiro-de-assassins-creed-sera-lancado-em-2022/) e [*"The Elder Scrolls 5: Skyrim"*](https://canaltech.com.br/games/the-elder-scrolls-5-skyrim-tera-jogo-de-tabuleiro-em-2022-200773/), ambos disponíveis em *Xbox* e *Playstation*, terão suas versões de tabuleiro lançadas em 2022.

Este trabalho de conclusão do curso tem o objetivo de analisar um *dataset* e construir visualizações gráficas a partir de seus dados. Uma das sugestões do curso foi a base "*Board Games*", disponibilizada pelo *Kaggle* e utilizada pelo *Tidytuesday* em um de seus eventos semanais que desafiam os usuários da linguagem R a construirem visualizações sobre um tema e assim promover discussões saudáveis. Decidi usá-la para a elaboração deste trabalho.

Exploraremos os dados e construiremos as visualizações para os chamados **"Solo Board Games"**, aqueles que permitem partidas com apenas um jogador. Destacaremos os jogos desta categoria que se diferenciam segundo critérios como: as melhores médias na avaliações de usuários, a posição no ranking dos melhores jogos, os mais citados nas listas de desejos e nas listas de mais procurados, os mais comprados e a faixa etária de acordo com a base analisada.
...
<div align=justify style="text-indent: 3em;"> A base *"Board Games"* é composta pelos arquivos *"ratings"* e *"details"* que trazem as seguintes variáveis:
</div>
...
<br>
<div align=justify style="text-indent: 3em;"> Após a união destas bases, obtivemos um *dataset* com `r nrow(jogos_tabuleiro)` linhas e `r ncol(jogos_tabuleiro)` variáveis de jogos com diversos temas, durações, mecânicas de jogos e com diferentes faixas etárias.
...
<div align=justify style="text-indent: 3em;"> Assim como os jogos digitais, alguns jogos de tabuleiro permitem que a partida seja realizada por apenas um jogador, o qual pode aproveitar momentos em que se encontra só para fugir da tecnologia e viver uma divertida aventura. 

No seguinte gráfico, conhecido como *"lollypop plot"*, temos os jogos com as melhores avaliações ordenados pela sua posição no ranking geral, que considera as notas médias bayesianas.

Relacionamos apenas os 20 jogos melhores avaliados entre aqueles com no mínimo 10.000 avaliações.
...
<div align=justify style="text-indent: 3em;">O gráfico abaixo, com 33 categorias de jogos, contém a relação dos jogos mais desejados pelos usuários com tempo máximo de duração de 160 minutos, adquiridos por mais de 5000 pessoas (legenda amarela) e os jogos do topo do ranking citados anteriormente (legenda azul), mas que não estão entre os mais desejados em suas categorias.
</div>
...
Para encontrar os jogos para jogadores "solo" com duração mínima inferior a 160 minutos disponíveis por idade, forneça a categoria e a idade para visualizar quais opções existem entre os mais desejados e sua posição no ranking de notas:
...
jogos_selecao01 <- c(                               # relação dos jogos citados neste trabalho.
    ...
    "Neuroshima Hex! 3.0", "Pandemic", "Prolix", "Reykholt",
...
<div align=justify style="text-indent: 3em;"> Você pode encontrar a descrição completa, selecionado o título para ler a descrição completa:
</div>
...
Realizada a análise dos jogos para usuários "solo", por meio das visualizações, podemos observar que nem todos os jogos do topo do *ranking* estão entre os mais desejados. 

Fatores como preço, tempo mínimo de jogo, categoria, mecânica do jogo, faixa etária afetam o desejo de ganhar ou comprar um jogo.

A existência de diversos temas, diferentes tempos de jogos, faixas etárias tornam difícil a escolha do melhor jogo para comprar.

Um tema que não chama a atenção do jogador o torna desinteressante, é preciso escolher bem entre os vários temas disponiveis como aventura, terror, médico, animais, ficção científica e até inspirados em outras obras como livros;  

O tempo de jogo é importante para quem joga sozinho, então  aquele que demore em média 4 horas talvez não seja a melhor opção; 

Se quem vai jogar é uma criança, deve-se observar a idade mínima. Muitos jogos exigem um raciocínio analítico e uso de estratégias avançadas, por este motivo os fabricantes informam a idade mínima recomendada.

Em uma era de video games, um jogo de tabuleiro pode-lhe ser uma agradável surpresa, então, não espere um apagão para se divertir!
